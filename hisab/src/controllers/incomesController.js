...
export async function createIncome(req, res) {
  ...
  try {
    ...
    // Update bank account balance if income is linked to one
    if (bankAccountId) {
      await client.query(
...
export async function deleteIncome(req, res) {
  ...
  try {
    ...
    const verifyQuery = `
      SELECT "bankAccountId", "amount" FROM hisab."incomes"
...
export async function updateIncome(req, res) {
  ...
  try {
    ...
    if (categoryId !== undefined) {
      const categoryCheck = await client.query(
        `SELECT "id" FROM hisab."incomeCategories" WHERE "id" = $1 AND "companyId" = $2`,
        [finalCategoryId, companyId]
      );
...
export async function getIncomes(req, res) {
  ...
  try {
    ...
    let query = `
      SELECT 
        i."id", i."date", i."amount", i."notes", i."createdAt", i."updatedAt",
